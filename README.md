...
#### 2.2.4 Объем прироста хранилища
...
Суточные метрики RPS
![image](https://user-images.githubusercontent.com/92160117/204089688-4e044914-4394-4ece-ba75-2235dc236cd3.png)
...
Определим пиковый сетевой трафик:

```
Сетевой трафик * соотношение = 2125,52 * 1,77 = 3762 Гб/с
```

## 3. Логическая схема базы данных

![Логическая схема БД](https://user-images.githubusercontent.com/92160117/202774441-815039d0-b3b0-4c19-9775-463b1d346cac.jpg)

Сущности базы данных:

* Пользователь - основные сведения о пользователе. PrimaryKey - UserID
* Пост - твит пользователя. PrimaryKey - PostID, содержит информацию о пользователе (Many2Many), содержании, количестве лайков, вложении (Many2Many), дате публикации, аватарка пользователя (Many2Many).
...
![Физическая схема БД](https://user-images.githubusercontent.com/92160117/204089987-39e9ce27-0f1a-4e62-94e1-7b2f5dd4c639.jpg)
...
Проставление лайков является самой нагруженной операцией данного сервиса. Лайк по сути является записью, образующей ключ - значение = UserID - PostID. Поэтому для хранения лайков предлагается использовать отдельную базу данных, что позволит увеличить производительность сервиса. Предлагается использовать NoSQL для быстрой записи и чтения данных. При больших объемах данных производительность Redis падает [[13]](https://aerospike.com/when-to-use-aerospike-vs-redis/), поэтому  был сделан выбор в сторону AeroSpike,  так как она ориентирована на достижение максимальной производительности на атомарных операциях, почти линейную масштабируемость и надежную согласованность данных.

## 4.3 Посты 

Таблица постов является самой большой в проекте, поэтому хранение этой таблицы на одном сервере не представляется возможным. Самыми нагруженными постами будут недавно опубликованные посты, поэтому можно использовать базу данных Cassandra, так как она масштабируема, высокопроизводительная, а также не имеет единой точки отказа.

Cassandra предполагает разделение на ноды - шардирование, а также реплицирование данных. Данные организованы по таблице и идентифицированы первичным ключом, который определяет, на каком узле хранятся данные. Это позволит хранить более свежие посты в определенной ноде, которая и будет наиболее загруженной. Также в данной NoSQL DB предусмотрено реплицирование данных для обеспечения отказоустойчивости и надежности.
...
Пользователей также, как и посты, будем хранить в Cassandra, так как она автоматически шардирует и реплицирует данные. Шардирование будет происходить по первичному ключу - UserID.
...
| Cassandra | Backend | Распределенная система управления базами данных. Высокая доступность без единой точки отказа, масштабируемость, отказоустойчивость, распределение данных |
...
![Схема проекта](https://user-images.githubusercontent.com/92160117/204089894-bd5ed77f-bb55-4529-b10f-811635f04ae2.jpg)
...
```
18 Пб / 5 Тб = 3,6к, учитывая запас для хранения логов, метрик и прочего - 10к бакетов
```

| Amazon S3 | Количество бакетов|
|-|-|
| Хранилище данных| 10 000 |
...
**Cassandra**
Основные часть занимаемых данных размещена в облаке, поэтому для выбора машин для Cassandra нужно сделать упор на скорость записи и чтения.

Используя данные статьи [[14]](https://netflixtechblog.com/benchmarking-cassandra-scalability-on-aws-over-a-million-writes-per-second-39f45f066c9e), можно определить, что Cassandra на 48 нодах может выдерживать порядка 174 w/s. Не все запросы к базе данных идут на запись, поэтому вполне вероятно, что меньшее количество более производииельных реплик справятся с нагрузкой.

| CPU | RAM (ГБ) | Диск (ТБ) | Количество |
|-|-|-|-|
| 12 | 128 | 5 | 10 - на посты, 5 - на пользователей |
...
По результатам тестирования [[15]](https://discuss.aerospike.com/t/some-questions-about-aerospike-performance-test/8352), с данной конфигурацией 3 сервера выдерживают 25к RPS на запись и 100к RPS на чтение, что практически удовлетворяет потребность нашего сервиса. Для обеспеченяи стабильности и отказоустойчивости, количество машин возьмем с запасом:
