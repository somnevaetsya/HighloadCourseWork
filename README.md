...
Твиттер генерирует порядка 12 ТерраБайт новых данных в день [[12]](https://www.loginradius.com/blog/engineering/handling-cheapest-fuel-data/). Тогда, нагрузка в секунду составит: `12 000 / 24 * 60 * 60 = 0.138 Gb/sec` 
...
![Физическая схема БД](https://user-images.githubusercontent.com/92160117/202774395-fe54d3d1-05f6-4e69-a39e-d3742cfb5475.jpg)
...
Проставление лайков является самой нагруженной операцией данного сервиса. Лайк по сути является записью, образующей ключ - значение = UserID - PostID. Поэтому для хранения лайков предлагается использовать отдельную базу данных, что позволит увеличить производительность сервиса. Предлагается использовать NoSQL базу данных Redis, так как она ориентирована на достижение максимальной производительности на атомарных операциях, а также позволяет горизонтально масштабироваться с помощью Redis Cluster.

Также необходимо предусмотреть реплицирование данных для обеспечения отказоустойчивости и надежности.
...
Запись сессии представляет собой аналогичную лайку пару ключ - значение, где ключом выступает SessionID, а значением - UserID. Для хранения таких записей также предлагается использовать NoSQL базу данных - Redis.
...
| PostgreSQL | Backend | Возможность шардирования и реплицирования, высокая производительность, обширный функционал |
...
![Схема проекта](https://user-images.githubusercontent.com/92160117/202774345-0911553a-33ad-408f-afdd-3af45f923b2f.jpg)
...
Основными данными являются медиафайлы, которые предполагается хранить в облачном хранилище Amazon S3. Максимальный размер бакета составляет 5 Тб [[12]](https://aws.amazon.com/ru/s3/faqs/#:~:text=S3%20%D0%BC%D0%BE%D0%B6%D0%B5%D1%82%20%D1%81%D0%BE%D1%81%D1%82%D0%B0%D0%B2%D0%BB%D1%8F%D1%82%D1%8C-,%D0%BE%D1%82%200%C2%A0%D0%B1%D0%B0%D0%B9%D1%82%20%D0%B4%D0%BE%205%C2%A0%D0%A2%D0%91,-.%20%D0%A1%D0%B0%D0%BC%D1%8B%D0%B9%20%D0%BA%D1%80%D1%83%D0%BF%D0%BD%D1%8B%D0%B9%20%D0%BE%D0%B1%D1%8A%D0%B5%D0%BA%D1%82). Рассчитаем количество бакетов:
...
Используя данные статьи [[13]](https://www.percona.com/blog/2017/01/06/millions-queries-per-second-postgresql-and-mysql-peaceful-battle-at-modern-demanding-workloads/), можно определить, что PostgreSQL может выдерживать порядка 40к TPS. Не все запросы к базе данных будут использовать транзакции, поэтому вполне вероятно, что несколько реплик с такой конфигурацией справятся с нагрузкой.

| CPU | RAM (ГБ) | Диск (ТБ) | Количество |
|-|-|-|-|
| 24 | 512 | 5 | 10 - на посты, 5 - на пользователей |

**Балансировка**

Nginx может выдерживать порядка 50к RPS [[14]](https://rohitgupta.xyz/blog/tuning-nginx-for-better-rps-of-an-http-api/). Для обеспечения отказоустойчивости и надежности предусмотрим несколько реплик.

| Балансировка | Количество |
|-|-|
| L7 | 5 |
